...
// AUTO
			for (let anji of setik){
				if (budy === anji){
					...
					bosco.sendMessage(from, result, sticker, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { orderMessage: { itemCount: 9999999, status: 200, thumbnail: fs.readFileSync('./ds.jpg'), surface: 200, message: `✘ ī.ᴀᴍ/ꪶ͢sᴋʏʟɪɴᴇꫂ ⁩✘ ➫➬➭ ${anji}`, orderTitle: `𒈒 ī.ᴀᴍ/ꪶ͢sᴋʏʟɪɴᴇꫂ⁩ 𒈒➫➬➭ ${anji}`, sellerJid: '0@s.whatsapp.net'}}}})
					}
			}
			for (let anju of vien){
				if (budy === anju){
					result = fs.readFileSync(`./media/vn/${anju}.mp3`)
					bosco.sendMessage(from, result, audio, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { orderMessage: { itemCount: 9999999, status: 200, thumbnail: fs.readFileSync('./ds.jpg'), surface: 200, message: `✘ ī.ᴀᴍ/ꪶ͢sᴋʏʟɪɴᴇꫂ⁩ ✘ ➫➬➭ ${anju}`, orderTitle: `𒈒 ī.ᴀᴍ/ꪶ͢sᴋʏʟɪɴᴇꫂ⁩ 𒈒➫➬➭  ${anju}`, sellerJid: '0@s.whatsapp.net'}}}, mimetype: 'audio/mp4', duration: 444444444444, ptt: true})
					}
			}
			for (let anjh of imagi){
				if (budy === anjh){
					result = fs.readFileSync(`./media/image/${anjh}.jpg`)
					bosco.sendMessage(from, result, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { orderMessage: { itemCount: 9999999, status: 200, thumbnail: fs.readFileSync('./ds.jpg'), surface: 200, message: `✘ ī.ᴀᴍ/ꪶ͢sᴋʏʟɪɴᴇꫂ⁩ ✘ ➫➬➭ ${anjh}`, orderTitle: `𒈒 ī.ᴀᴍ/ꪶ͢sᴋʏʟɪɴᴇꫂ⁩ 𒈒➫➬➭ ${anjh}`, sellerJid: '0@s.whatsapp.net'}}}})
					}
			}
      ...
      const sendFileFromUrl = async(link, type, options) => {
          ...
          if (!packname) packname = '!Ameen'; if (!author) author = 'Ser';author = author.replace(/[^a-zA-Z0-9]/g, '');	
...
//------------------< Bot Owner >-------------------
       ...
       case 'group':
        rows = [
          ...
          section = [{title: "ғᴏʟʟᴏᴡ xʏ_ʟxx_ ᴏɴ ɪɴsᴛᴀ 🎗️", rows: rows}]
...
`
 ...
 menumessage = { contentText: `${teks}`, footerText: `Bᴏᴛ ʙʏ ᴀᴍᴇᴇɴ sᴇʀ`, buttons: menubutton, headerType: 6, locationMessage: bosco2.message.locationMessage}
 ...
 //------------------< Extra >-------------------
      ...
      case 'tod':
        ...
        case 'anime':
              ...
              imageMsg = ( await bosco.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'© 𝒎𝒂𝒅𝒆 𝒃𝒚 ᴀᴍᴇᴇɴ sᴇʀ', imageMessage: imageMsg,
...
//------------------< Storage >-------------------
	 ...
	 case 'delvn':
				...
				case 'listbgm':
					...
					teks += `\n*Tᴏᴛᴀʟ : ${vien.length}*\n\n_ʜᴇʏ ʜᴇʀᴇ ɪs ᴛʜᴇ ʙɢᴍ ʟɪsᴛ_`
					bosco.sendMessage(from, teks.trim(), extendedText, { caption: 'teks', "contextInfo": { text: 'teks', sendEphemeral: true, "externalAdReply": { "title": `${' '}𝐀𝐌𝐄𝐄𝐍 𝐒𝐄𝐑${''}${''}`, "body": ``, "previewType": 'PHOTO', "thumbnailUrl": `${'https://i.ibb.co/vkkcm0L/034c588fd8d5.jpg'}`, "thumbnail": '', "sourceUrl": `${'https://wa.me/c/62852407507130'}`}},quoted: ftext})
					break
				case 'addimage':
